...
class DetectionController extends ChangeNotifier {
  DetectionController({
    ...
    _livenessSteps = List.of(config.steps);
  ...
  /// Called once when the flow ends, with the final [KycResult] (outcome status
  /// plus every captured image, each tagged with its capture type/step).
  final void Function(KycResult result) onFinished;
  ...
  double get overallProgress {
    ...
    return total == 0 ? 0 : (_completedCaptures / total).clamp(0.0, 1.0);
  }

  //* MARK: - Private State
  //? =========================================================
  FacePhase _phase = FacePhase.initializing;
  FaceValidationResult _validation = const FaceValidationResult.invalid(
    FaceIssue.noFace,
  );
  ...
  final List<CapturedImage> _captured = [];
  ...
  void _handleDistancePhase(List<Face> faces, {required bool isNear}) {
    _stable.requiredDuration = isNear
        ? config.nearStableDuration
        : config.farStableDuration;
    final FaceValidationResult result = _validate(faces, isNear: isNear);
    _validation = result;
    _stable.update(result.isValid);

    final FacePhase holding = isNear
        ? FacePhase.stabilizingNear
        : FacePhase.stabilizingFar;
    final FacePhase waiting = isNear
        ? FacePhase.moveCloser
        : FacePhase.positioningFar;
  ...
  FaceValidationResult _validate(List<Face> faces, {required bool isNear}) {
    ...
    if (!FaceDetectionService.isStraight(face, tolerance: isNear ? 22 : 18)) {
      return FaceValidationResult.invalid(
        FaceIssue.notStraight,
        metric: metric,
      );
    }
  ...
  Future<void> _capture({required bool isNear}) async {
    ...
    _addCapture(
      path,
      type: isNear ? KycCaptureType.near : KycCaptureType.far,
      auto: true,
    );
  ...
  void _detectTurn(Face face, {required bool toLeft}) {
    ...
    final bool turned = wantPositiveYaw ? yaw > threshold : yaw < -threshold;
  ...
  Future<void> _completeLivenessStep() async {
    ...
    _addCapture(
      path,
      type: KycCaptureType.liveness,
      auto: true,
      step: _livenessSteps[_livenessIndex],
    );
    _didCloseEyes = false;
    _livenessIndex++;
  ...
  //? =========================================================
  void _startTimeout() {
    final int? maxSec = config.maxSecToDetect;
    if (maxSec == null) return; // No time limit.
    _timeout = Timer(Duration(seconds: maxSec), () {
      if (_finished) return;
      if (config.allowAfterMaxSec) {
        _manualCaptureVisible = true;
        notifyListeners();
      } else {
        _finish(KycStatus.timeout);
      }
    });
  }
  ...
  Future<void> manualCapture() async {
    ...
    notifyListeners();
    final String? path = await capture.takePhoto();
    _addCapture(path, type: KycCaptureType.manual, auto: false);
    _busy = false;
    _finish(KycStatus.success);
  }
  ...
  //* MARK: - Completion
  //? =========================================================
  /// Records a captured image (skipping failed captures) and advances the
  ...
  void _finish(KycStatus status) {
    if (_finished) return;
    _finished = true;
    _phase = FacePhase.completed;
    _timeout?.cancel();
    notifyListeners();
    onFinished(KycResult(status: status, images: List.unmodifiable(_captured)));
  }
}
